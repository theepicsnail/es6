...
/***/ function(module, exports) {
	...
	window.setInterval(function () {
	  ...
	  factor = .03 * (Math.sin(t / 50) - Math.sin(t / 30)) / 2 + 1;
	  patternCtx.drawImage(canvas, 0, 0);
	
	  ctx.save();
	  ctx.translate(canvas.width / 2, canvas.height / 2);
	  ctx.rotate(Math.PI / 180 * (Math.sin(t / 7 + Math.sin(t / 9)) + Math.cos(t / 9 - Math.cos(t / 7))));
	  ctx.translate(-canvas.width / 2, -canvas.height / 2);
	  ...
	  ctx.drawImage(patternCanvas, canvas.width * f2, canvas.height * f2, canvas.width * factor, canvas.height * factor);
	  ctx.globalAlpha = 1;
